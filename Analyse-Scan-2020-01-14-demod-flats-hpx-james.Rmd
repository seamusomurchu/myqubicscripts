...
print(min(newaz), max(newaz), min(newel), max(newel))
print(len(az), len(el), len(newaz), len(newel))
```
...
```{python}
del errors_demod, okdemod, oktime_azel, newdata, oktime_d, oktime_s, time_azel
```
...
fa(elmap).save('/home/james/synthbeamdata/2020-03-14/Flat/elevation.fits')
```
...
```{python}
print(newaz.shape, newel.shape, azmap.shape, elmap.shape)
```
...
for i in range(256):
    ...
    FitsArray(mapdata[i,:,:]).save('/home/james/synthbeamdata/2020-03-16/Flat/imgflat_TESNum_{}.fits'.format(i+1))
```
